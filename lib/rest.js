...
var id_format = exports.id_format = /^[0-9a-f]{24}$/;
...
exports.autoloadResources = function (app, config, ignore) {
    config = config || {};
    ignore = ignore || [];

    models.getTopLevel().forEach(function (resource) {
        if (ignore.indexOf(resource) !== -1) return;
...
function addRoutes (app, routes, prefix, singular) {
    ...
    if ('destroy' in routes)
        app.delete(prefix + '/:' + singular, routes.destroy);
}

/**
 * Create resource-based, RESTful routes for top-level mongoose models.
...
exports.create = function (app, config, include_routes, ignore) {
    ignore = ignore || [];
    ...
    models.getTopLevel().forEach(function (resource) {
        ...
        //Add RESTful routes
        routes = topLevelRoutes(resource, config);

        //Only include some routes?
        if (include_routes) {
            for (var type in routes) {
                if (!~include_routes.indexOf(type)
                        && !~include_routes.indexOf(singular + '.' + type)) {
                    delete routes[type]
                }
            }
        }
...
/**
 * Create resource-based, RESTful routes for embedded mongoose models.
 * All models must be defined before calling this method.
 *
 * @param {HTTPServer} app
 * @param {object} config (optional)
...
exports.createEmbedded = function (app, config, include_routes, ignore) {
    ignore = ignore || [];
    ...
    models.getTopLevel().forEach(function (resource) {
        ...
        models.getChildren(resource).forEach(function (embedded) {
            ...
            routes = embeddedRoutes(resource, embedded.resource, embedded.attribute);

            //Only include some routes?
            if (include_routes) {
                for (var type in routes) {
                    if (!~include_routes.indexOf(type) &&
                            !~include_routes.indexOf(embedded.singular + '.' + type)) {
                        delete routes[type]
                    }
                }
            }
...
exports.getRoutes = function (config) {
    var routes = {};

    //Create routes for each top level model
    models.getTopLevel().forEach(function (resource) {
        routes[resource] = topLevelRoutes(resource, config);

        routes[resource].embedded = {};

        //Create routes for embedded documents
        models.getChildren(resource).forEach(function (embedded) {
            routes[resource].embedded[embedded.attribute] = embeddedRoutes(resource,
...
function topLevelRoutes (resource, config) {
    ...
    routes.create = function (request, response, next) {
        ...
        instance.save(function (err) {
            ...
            if (err) {
                ...
                response.header('location', '/' + plural + '/' + id);
                return response.send(201);
            }
            request.flash('success', 'The %s was created successfully.', singular);
    ...
    routes.update = function (request, response, next) {
        ...
        instance.save(function (err) {
            ...
            request.flash('success', 'The %s was updated successfully.', singular);
            var redirect = request.redirect
                         ? request.redirect
                         : '/' + plural + '/' + request.params[singular];
            response.redirect(redirect);
        });
    }
    ...
    routes.destroy = function (request, response, next) {
        request.resource(singular).remove(function (err) {
            ...
            request.flash('success', 'The %s was removed successfully.', singular);
            var redirect = request.redirect
                         ? request.redirect
                         : '/' + plural;
            response.redirect(redirect);
